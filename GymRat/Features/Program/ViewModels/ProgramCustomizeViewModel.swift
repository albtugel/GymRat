...
@Observable
@MainActor
final class ProgramCustomizeViewModel {

    // MARK: - State
    private(set) var mode: ProgramCustomizeMode
    private(set) var program: Program
    private(set) var programName: String
    private(set) var programColorHex: String
    private(set) var selectedExercises: [ProgramExercise]
    private(set) var selectedWeekdays: Set<ProgramWeekDay>
    private(set) var customExercises: [ExerciseModel] = []
    ...
    private(set) var pendingSeed: ExerciseStore.ExerciseSeed?
    ...
    private let exerciseLogService: ProgramExerciseLogServiceProtocol
    ...
    init(
        ...
        exerciseLogService: ProgramExerciseLogServiceProtocol,
        ...
        self.selectedWeekdays = ProgramModelMapper.weekdays(for: program)
    ...
    var programType: ProgramType {
        ProgramModelMapper.type(for: program)
    }

    var programTitle: String {
        ProgramTypeDisplay.title(for: programType)
    }
    ...
    var filteredExerciseSeeds: [ExerciseStore.ExerciseSeed] {
        var seeds = exerciseStore.seeds
            .filter { ProgramTypeDisplay.allows(programType, category: $0.category) }

        let custom = customExercises
            .filter { $0.isCustom && ProgramTypeDisplay.allows(programType, category: ExerciseModelMapper.category(for: $0)) }
            .map {
                let category = ExerciseModelMapper.category(for: $0)
    ...
    func resolveSelectionInfo(for seed: ExerciseStore.ExerciseSeed) -> ExerciseSelectionInfo {
        guard let index = selectedExercises.firstIndex(where: { $0.exercise.name == seed.name }) else {
            return ExerciseSelectionInfo(selectedExercise: nil, selectionNumber: nil)
        }
        let number = isEditing ? nil : index + 1
        return ExerciseSelectionInfo(selectedExercise: selectedExercises[index], selectionNumber: number)
    }
    ...
    func toggleExercise(_ seed: ExerciseStore.ExerciseSeed) async {
    ...
    func createCustomExercise() async {
        ...
        let newExercise = ExerciseModel(name: trimmed, categoryRaw: newExerciseCategory.rawValue, isCustom: true)
        do {
    ...
    func saveProgram() async {
        ...
        ProgramModelMapper.setWeekdays(selectedWeekdays, for: program)

        guard await persistProgram() else { return }
        await finishSave()
    }

    func clearHistory(for exercise: ProgramExercise) async {
        do {
    ...
    // MARK: - Helpers
    private func fetchOrCreateExercise(_ seed: ExerciseStore.ExerciseSeed) async -> ExerciseModel? {
        do {
            if let existing = try await exerciseService.fetchExercise(named: seed.name) {
                return existing
            }
            let newExercise = ExerciseModel(name: seed.name, categoryRaw: seed.category.rawValue)
            try await exerciseService.addExercise(newExercise)
            customExercises = (try await exerciseService.fetchExercises()).filter { $0.isCustom }
            return newExercise
        } catch {
    ...
    // MARK: - Helpers
    private func addExercise(from seed: ExerciseStore.ExerciseSeed, sharedHistory: Bool) async {
        guard let exercise = await fetchOrCreateExercise(seed) else { return }
    ...
    private func appendSelectedExercise(_ exercise: ExerciseModel, sharedHistory: Bool) {
        selectedExercises.append(ProgramExercise(exercise: exercise, sharedHistory: sharedHistory))
    }

    private func updateSharedHistoryIfNeeded(_ sharedHistory: Bool, exercise: ExerciseModel) async {
        guard sharedHistory else { return }
        applySharedHistory(for: exercise)
        await saveProgramSilently()
    }

    private func applySharedHistory(for exercise: ExerciseModel) {
        allPrograms
            .filter { $0.id != program.id }
            .flatMap { $0.exercises }
    ...
    private func updateProgramExercises() {
        if mode == .edit {
    ...
    private func makeOrderedExercises() -> [ProgramExercise] {
    ...
    private func existingSelectedExercises() -> [ProgramExercise] {
        let selectedIds = Set(selectedExercises.map { $0.id })
        return program.exercises.filter { selectedIds.contains($0.id) }
    }

    private func newSelectedExercises(from ordered: [ProgramExercise]) -> [ProgramExercise] {
        let existingIds = Set(ordered.map { $0.id })
        return selectedExercises.filter { !existingIds.contains($0.id) }
    }

    private func applySelectionIndicesIfNeeded(for exercises: [ProgramExercise], existing: [ProgramExercise]) {
