...
struct ProgramCustomizeCreateExerciseButton: View {
    ...
    var body: some View {
        ...
        .alert(LocalizedStringKey(AppAlerts.CreateExercise.title), isPresented: createAlertBinding) {
            TextField("exercise_name_placeholder", text: newExerciseNameBinding)
                .autocorrectionDisabled()
            Picker("", selection: newExerciseCategoryBinding) {
                ForEach(ExerciseCategory.allCases, id: \.self) { cat in
                    Text(ExerciseCategoryDisplay.localizedLabel(for: cat)).tag(cat)
                }
            }
            Button("save_button") {
