...
@Observable
@MainActor
final class ProgramTableExerciseRowViewModel {
    private struct LogContext {
        let day: Date
        let dayStamp: Int
        let currentLog: ProgramExerciseLog?
        let previousLog: ProgramExerciseLog?
    }
    ...
    private var currentLog: ProgramExerciseLog?
    ...
    // MARK: - Dependencies
    private let logService: ProgramExerciseLogServiceProtocol
    ...
    init(
        ...
        selectedDate: Date,
        logService: ProgramExerciseLogServiceProtocol,
    ...
    var isCardio: Bool {
        ExerciseModelMapper.category(for: programExercise.exercise) == .cardio
    }
    ...
    func handleFocusChange(_ newValue: ProgramTableFocusField?) async {
    ...
    // MARK: - Bindings
    func setsCountTextValue() -> String {
        setsCountText
    }
    ...
    private func buildLogContext() async -> LogContext {
        ...
        let dayStamp = ProgramExerciseLogHelper.makeDayStamp(for: day)
    ...
    private func fetchLogs() async -> [ProgramExerciseLog] {
    ...
    private func selectPreviousLog(from logs: [ProgramExerciseLog], dayStamp: Int) -> ProgramExerciseLog? {
        logs
    ...
    private func selectCurrentLog(
        from logs: [ProgramExerciseLog],
        dayStamp: Int
    ) async -> ProgramExerciseLog? {
    ...
    private func deleteDuplicateLogs(_ logs: ArraySlice<ProgramExerciseLog>) async {
        for log in logs {
    ...
    private func makeSetsCountText(
        currentLog: ProgramExerciseLog?,
        previousLog: ProgramExerciseLog?
    ) -> String {
    ...
    private func applyCurrentLog(_ log: ProgramExerciseLog?) {
        if let log {
    ...
    private func applyPreviousLog(_ log: ProgramExerciseLog?) {
        if let log {
    ...
    private func normalizeLogDatesIfNeeded(_ logs: [ProgramExerciseLog]) async {
        var didChange = false
        for log in logs {
            let normalized = ProgramExerciseLogHelper.startOfDay(for: log.date)
            let stamp = ProgramExerciseLogHelper.makeDayStamp(for: normalized)
    ...
    private func fetchLog(for dayStamp: Int) async -> ProgramExerciseLog? {
    ...
    private func syncCurrentLog(dayStamp: Int, logForDay: ProgramExerciseLog?) async {
        let dataStamp = ProgramExerciseLogHelper.makeDayStamp(for: dataDate)
    ...
    private func formatWeightValue(_ value: Double) -> String {
        guard value > 0 else { return "" }
        return formatWeight(isCardio ? value : unitsManager.displayWeight(value))
    }
    ...
    private func isRowFocused(_ field: ProgramTableFocusField?) -> Bool {
        switch field {
        case .sets(let id):
    ...
    private func logScore(_ log: ProgramExerciseLog) -> Int {
    ...
    private func hasValues(_ log: ProgramExerciseLog) -> Bool {
